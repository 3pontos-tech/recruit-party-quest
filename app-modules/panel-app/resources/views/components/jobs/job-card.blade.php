...
<x-he4rt::card
    class="group"
    href="{{ $job->post ? He4rt\App\Filament\Resources\JobRequisitions\JobRequisitionResource::getUrl('view', ['record' => $job->post->slug]) : '#' }}"
>
    <x-slot:header class="gap-4">
        <x-he4rt::avatar
            :src="$job->team->getFirstMediaUrl('logo', 'thumb') ?: asset('images/3pontos/logo-chain-white.png')"
            :alt="$job->team->name"
            size="lg"
            :circular="false"
