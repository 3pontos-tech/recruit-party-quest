<?php

declare(strict_types=1);

return [
    'relation_managers' => [
        'stages' => [
            'title' => 'Etapas do Pipeline',
            'label' => 'Etapa',
            'plural_label' => 'Etapas',
        ],
    ],
    'actions' => [
        'workflow' => [
            'label' => 'Ações',
        ],
        'approve' => [
            'label' => 'Aprovar',
            'modal_heading' => 'Aprovar Requisição',
            'modal_description' => 'Tem certeza que deseja aprovar esta requisição de vaga?',
            'modal_submit' => 'Aprovar',
            'notification' => 'Requisição de vaga aprovada com sucesso.',
        ],
        'publish' => [
            'label' => 'Publicar',
            'modal_heading' => 'Publicar Requisição',
            'modal_description' => 'Tem certeza que deseja publicar esta requisição de vaga? Ela ficará visível para os candidatos.',
            'modal_submit' => 'Publicar',
            'notification' => 'Requisição de vaga publicada com sucesso.',
        ],
        'put_on_hold' => [
            'label' => 'Pausar',
            'modal_heading' => 'Pausar Requisição',
            'modal_description' => 'Tem certeza que deseja pausar esta requisição de vaga?',
            'modal_submit' => 'Pausar',
            'notification' => 'Requisição de vaga pausada com sucesso.',
        ],
        'close' => [
            'label' => 'Encerrar',
            'modal_heading' => 'Encerrar Requisição',
            'modal_description' => 'Tem certeza que deseja encerrar esta requisição de vaga?',
            'modal_submit' => 'Encerrar',
            'notification' => 'Requisição de vaga encerrada com sucesso.',
        ],
        'cancel' => [
            'label' => 'Cancelar',
            'modal_heading' => 'Cancelar Requisição',
            'modal_description' => 'Tem certeza que deseja cancelar esta requisição de vaga? Esta ação não pode ser desfeita.',
            'modal_submit' => 'Cancelar Requisição',
            'notification' => 'Requisição de vaga cancelada com sucesso.',
        ],
        'clone' => [
            'label' => 'Duplicar',
            'modal_heading' => 'Duplicar Requisição',
            'modal_description' => 'Isso criará uma cópia da requisição com status Rascunho.',
            'modal_submit' => 'Duplicar',
            'notification' => 'Requisição de vaga duplicada com sucesso.',
        ],
    ],
    'stage' => [
        'fields' => [
            'name' => 'Nome',
            'stage_type' => 'Tipo de Etapa',
            'display_order' => 'Ordem de Exibição',
            'description' => 'Descrição',
            'expected_duration_days' => 'Duração Esperada (Dias)',
            'active_participants' => 'Participantes ativos',
            'active' => 'Ativo',
            'hidden' => 'Etapa Oculta?',
        ],
    ],
    'requisition' => [
        'label' => 'Requisição de Vaga',
        'plural_label' => 'Requisições de Vagas',
        'navigation_label' => 'Buscar Vagas',
        'tabs' => [
            'basic_information' => 'Informações Básicas',
            'job_description' => 'Descrição da Vaga',
            'position_details' => 'Detalhes da Posição',
            'requirements' => 'Requisitos',
            'compensation' => 'Remuneração',
            'settings' => 'Configurações',
            'job_posting' => 'Vaga',
            'misc' => 'Variados',
            'details' => 'Detalhes da Vaga',
            'screening_questions' => 'Perguntas de Triagem',
        ],
        'sections' => [
            'basic_information' => 'Informações Básicas',
            'basic_information_description' => 'Forneça informações básicas sobre a requisição de vaga',
            'position_details' => 'Detalhes da Posição',
            'position_details_description' => 'Forneça detalhes específicos sobre a posição',
            'team_ownership' => 'Liderança e Status',
            'job_posting' => 'Vaga',
            'external_posting' => 'Postagem Externa',
            'compensation' => 'Remuneração',
            'settings' => 'Configurações',
            'job_posting_details' => 'Detalhes da Vaga',
            'job_description' => 'Descrição da Vaga',
            'required_qualifications' => 'Qualificações Obrigatórias',
            'required_qualifications_description' => 'Habilidades, experiência e qualificações exigidas para esta posição',
            'preferred_qualifications' => 'Qualificações Desejáveis',
            'preferred_qualifications_description' => 'Habilidades interessantes que dariam uma vantagem aos candidatos',
            'responsibilities' => 'Responsabilidades',
            'responsibilities_description' => 'Principais deveres e responsabilidades para esta função',
            'benefits' => 'Benefícios',
            'benefits_description' => 'Vantagens, benefícios e o que você oferece aos candidatos',
            'company_info' => 'Informações da Empresa',
            'section_description' => 'Forneça uma descrição detalhada da função e o que a posição envolve',
        ],
        'fields' => [
            'id' => 'ID',
            'slug' => 'Slug',
            'team' => 'Empresa',
            'department' => 'Departamento',
            'department_description' => 'A equipe ou divisão à qual esta função pertence',
            'hiring_manager' => 'Gestor Responsável',
            'hiring_manager_description' => 'O recrutador oficial que possui esta requisição de trabalho.',
            'recruiter' => 'Gerente da Vaga',
            'status' => 'Status',
            'priority' => 'Prioridade',
            'work_arrangement' => 'Regime de Trabalho',
            'employment_type' => 'Tipo de Contratação',
            'employment_type_description' => 'A natureza do contrato de trabalho',
            'experience_level' => 'Nível de Experiência',
            'category' => 'Categoria',
            'category_description' => 'A área funcional ou setor desta posição',
            'positions_available' => 'Vagas Disponíveis',
            'positions_available_description' => 'Quantas vagas devem estar disponíveis',
            'work_arrangement_description' => 'Onde e como o colaborador irá trabalhar',
            'experience_level_description' => 'Senioridade exigida para esta posição',
            'status_description' => 'Rascunho? Em revisão? Publicada?',
            'priority_description' => 'Com que urgência precisamos fechar esta vaga?',
            'salary_range' => 'Faixa Salarial',
            'salary_range_min' => 'Salário Mínimo',
            'salary_range_max' => 'Salário Máximo',
            'salary_currency' => 'Moeda',
            'show_salary_to_candidates' => 'Mostrar Salário aos Candidatos',
            'is_internal_only' => 'Apenas Interno',
            'is_confidential' => 'Confidencial',
            'auto_screening_transition' => 'Triagem automática de candidatos',
            'auto_screening_transition_help' => 'Reprova candidatos que não atendem aos critérios da triagem e avança quem atende, com base nas respostas do questionário. A reprovação não é imediata: quem é reprovado só é desqualificado 1 dia após se candidatar, deixando uma janela para revisar ou reverter a decisão.',
            'target_start_at' => 'Data de Início Prevista',
            'post_title' => 'Título da Vaga',
            'applications_count' => 'Candidaturas',
            'published_at' => 'Publicado em',
            'created_at' => 'Criado em',
            'updated_at' => 'Atualizado em',
            'deleted_at' => 'Excluído em',
        ],
        'job_posting' => [
            'label' => 'Vaga',
            'plural_label' => 'Vagas',
            'fields' => [
                'title' => 'Título',
                'slug' => 'Slug',
                'summary' => 'Resumo',
                'description' => 'Descrição',
                'responsibilities' => 'Responsabilidades',
                'required_qualifications' => 'Qualificações Obrigatórias',
                'preferred_qualifications' => 'Qualificações Desejáveis',
                'benefits' => 'Benefícios',
                'content' => 'Conteúdo',
                'tags' => 'Tags',
                'about_company' => 'Sobre a Empresa',
                'about_team' => 'Sobre o Time',
                'work_schedule' => 'Horário de Trabalho',
                'accessibility_accommodations' => 'Acomodações de Acessibilidade',
                'is_disability_confident' => 'Empregador Deficiente Confiante',
                'external_post_url' => 'URL da Vaga Externa',
            ],
            'helpers' => [
                'one_per_line' => 'Digite um item por linha',
            ],
            'notifications' => [
                'successful' => 'Vaga criada com sucesso.',
                'generating' => 'Estamos gerando sua vaga, aguarde alguns segundos.',
                'failed' => 'Algo deu errado, vaga não foi gerada.',
            ],
        ],
        'filters' => [
            'status' => 'Status',
            'priority' => 'Prioridade',
            'work_arrangement' => 'Regime de Trabalho',
            'employment_type' => 'Tipo de Contratação',
            'experience_level' => 'Nível de Experiência',
            'category' => 'Categoria',
            'team' => 'Empresa',
            'department' => 'Departamento',
            'is_internal_only' => 'Apenas Interno',
            'is_confidential' => 'Confidencial',
        ],
        'kanban' => [
            'nav' => [
                'edit_stages' => 'Editar Etapas',
                'edit_label' => 'Editar',
                'kanban_stages' => 'Etapas do Kanban',
            ],
            'actions' => [
                'view_candidate' => [
                    'label' => 'Ver Informações',
                ],
            ],
            'sections' => [
                'candidate_information' => 'Informações do Candidato',
                'application_comments' => 'Comentários da Candidatura',
            ],
        ],
    ],
];
