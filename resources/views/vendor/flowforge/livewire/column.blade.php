        ...
        <div class="flex flex-col w-full gap-1">
            <div class="flex justify-between">
                <div class="flex items-center">
                    @if ($column['icon'] ?? null)
                        <x-filament::icon :icon="$column['icon']"
                                          class="h-4 w-4 text-gray-500 dark:text-gray-400 me-2" />
                    @endif
                    ...
                    @else
                        ...
                        <div
                            class="ms-2 items-center border px-2 py-0.5 rounded-md text-xs font-semibold bg-gray-50 dark:bg-gray-600/20 text-gray-700 dark:text-gray-300 border-gray-700/30 dark:border-gray-300/30">
                            {{ $column['total'] ?? (isset($column['items']) ? count($column['items']) : 0) }}
                        </div>
                    @endif
                </div>
            ...
            <span class="text-xs">
                @if($column['description'] instanceof BackedEnum)
                    @php
                        $enum = $column['description'];
                    @endphp
                    <x-filament::badge :color="$enum->getColor()">
                        {{ $enum->getLabel() }}
                    </x-filament::badge>
                @else
                    {{ $column['description'] }}
                ...
                <div class="flex items-center gap-1">
                    <div class="flex -space-x-2">
                        @foreach($column['recruiters'] as $recruiter)
                            <span data-slot="tooltip-trigger"
                                  class="relative flex size-8 shrink-0 overflow-hidden rounded-full h-7 w-7 border-3 border-background cursor-pointer hover:z-10 hover:scale-110 transition-transform"
                                  data-state="closed">
                        <x-filament::avatar src="{{ $recruiter->user->getFilamentAvatarUrl() }}" />
                    </span>
                        @endforeach
                    </div>
                </div>
